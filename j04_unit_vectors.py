    ...
    def construct(self):
        ...
        class VecGroup:
            ...
            def makeMatrix(self, orientation=UP):
                mat = Matrix([[self.end[0]-self.start[0]], [self.end[1]-self.start[1]]], z_index=2)
        ...
        startUp("Unit Vectors", 4.8) #Sets up the background of the video.
        grid = setGrid(-3, -1, 6, 5) #Sets up the grid for the video. The Grid object can now be referred to by the grid variable.
        
        
        #START CODE HERE:
        vec1 = VecGroup([0, 0], [4, 3], fill=GREEN, shift=DOWN, show_coords=False)
        vec1.addVecGroup(anim)
        
        sym1 = Group()
        sym1.add(Tex(r"$\vec{u}$ = ", color=GREEN, font_size=96))
        sym1[0].move_to(grid.c2p(-3, 4))
        sym1.add(Matrix([[4], [3]]))
        sym1[1].set_fill(GREEN)
        sym1[1].move_to(grid.c2p(-1.5, 4))
        dynAdd(sym1, anim)
        
        dynWait(2, anim)
        
        sym1_1 = Group()
        sym1_1.add(Tex("$||$" + r"$\vec{u}$" + "$||$" + " = ", color=GREEN, font_size=64))
        sym1_1[0].move_to(grid.c2p(-3.75, 4))
        sym1_1.add(Tex(r"$\sqrt{ u_1 \cdot u_2 }$", font_size=48))
        sym1_1[1].set_fill(GREEN)
        sym1_1[1].move_to(grid.c2p(-1.75, 4))
        dynReplace(sym1, sym1_1, anim)
        
        dynWait(2, anim)
        
        sym1_2 = Group()
        sym1_2.add(Tex("$||$" + r"$\vec{u}$" + "$||$" + " = ", color=GREEN, font_size=64))
        sym1_2[0].move_to(grid.c2p(-3.75, 4))
        sym1_2.add(Tex(r"$\sqrt{ 4^2 + 3^2 }$", font_size=48))
        sym1_2[1].set_fill(GREEN)
        sym1_2[1].move_to(grid.c2p(-1.75, 4))
        dynReplace(sym1, sym1_2, anim)
        
        dynWait(2, anim)
        
        sym1_3 = Group()
        sym1_3.add(Tex("$||$" + r"$\vec{u}$" + "$||$" + " = ", color=GREEN, font_size=64))
        sym1_3[0].move_to(grid.c2p(-3.75, 4))
        sym1_3.add(Tex(r"$\sqrt{ 25 }$", font_size=48))
        sym1_3[1].set_fill(GREEN)
        sym1_3[1].move_to(grid.c2p(-1.75, 4))
        dynReplace(sym1, sym1_3, anim)
        
        dynWait(2, anim)
        
        sym1_3 = Group()
        sym1_3.add(Tex("$||$" + r"$\vec{u}$" + "$||$" + " = ", color=GREEN, font_size=64))
        sym1_3[0].move_to(grid.c2p(-3.25, 4))
        sym1_3.add(Tex("5", font_size=48))
        sym1_3[1].set_fill(GREEN)
        sym1_3[1].move_to(grid.c2p(-1.75, 4))
        dynReplace(sym1, sym1_3, anim)
        ...
        sym2 = Group()
        sym2.add(Tex(r"$\hat{u}$ =", color=YELLOW, font_size=96))
        sym2[0].move_to(grid.c2p(-3, 1.5))
        t1 = Tex(r"$\frac{4}{" + r"||\vec{u}||" + "}$")
        t2 = Tex(r"$\frac{3}{" + r"||\vec{u}||" + "}$")
        sym2.add(MobjectMatrix([t1, t2]))
        sym2[1].set_fill(YELLOW)
        sym2[1].move_to(grid.c2p(-1.5, 1.5))
        dynAdd(sym2, anim)
        
        dynWait(2, anim)
        
        sym2_1 = Group()
        sym2_1.add(Tex(r"$\hat{u}$ =", color=YELLOW, font_size=96))
        sym2_1[0].move_to(grid.c2p(-3, 1.5))
        t1 = Tex(r"$\frac{4}{5}$")
        t2 = Tex(r"$\frac{3}{5}$")
        sym2_1.add(MobjectMatrix([t1, t2]))
        sym2_1[1].set_fill(YELLOW)
        sym2_1[1].move_to(grid.c2p(-1.5, 1.5))
        dynReplace(sym2, sym2_1, anim)
        
        dynWait(2, anim)
        
        vec2 = VecGroup([0, 0], [4/5, 3/5], fill=YELLOW, show_coords=False)
