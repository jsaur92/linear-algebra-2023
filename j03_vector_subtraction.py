...
class VectorSubtraction(Scene):
    def construct(self):
        ...
        vec3 = VecGroup([1, 1], [3, 0], fill=ORANGE, orientation=LEFT, show_coords=False)
        vec3.addVecGroup(anim, extend=["subtract", [3, 2], [1, 3]])
        
        #representation for subtracting the vectors
        for i in sym2:
            sym1.add(i)
        ...
        sym5 = Group()
        sym5.add(IntegerMatrix([[3], [2]]))
        sym5[0].set_fill(RED)
        sym5[0].move_to(grid.c2p(-4.5, 3))
        sym5.add(Text("-", color=GRAY, font_size=64))
        sym5[1].move_to(grid.c2p(-3, 3))
        sym5.add(IntegerMatrix([[1], [3]]))
        sym5[2].set_fill(BLUE)
        sym5[2].move_to(grid.c2p(-1.5, 3))
        
        sym6 = Group()
        sym6.add(IntegerMatrix([[3], [2]]))
        sym6[0].set_fill(RED)
        sym6[0].move_to(grid.c2p(-4.5, 3))
        sym6.add(Text("+", color=GRAY, font_size=64))
        sym6[1].move_to(grid.c2p(-3, 3))
        sym6.add(IntegerMatrix([[-1], [-3]]))
        sym6[2].set_fill(YELLOW)
        sym6[2].move_to(grid.c2p(-1.5, 3))
        
        t1 = str(3) + " - " + str(1)
        t2 = str(2) + " - " + str(3)
        sym7 = Matrix([[t1], [t2]], z_index=2)
        sym7.move_to(grid.c2p(-3, 3))
        sym7.set_fill(ORANGE)
        
        sym8 = IntegerMatrix([[2], [-1]], z_index=2)
        sym8.move_to(grid.c2p(-3, 3))
        sym8.set_fill(ORANGE)
        
        dynAdd(sym3, anim)
        dynAdd(sym5, anim)
        dynWait(1, anim)
        dynReplace(sym3, sym4, anim)
        dynReplace(sym5, sym6, anim)
        dynWait(1, anim)
        dynReplace(sym5, sym7, anim)
        dynWait(1, anim)
